...
import org.scadalts.modbuspal2.automation.NullAutomation;
import org.scadalts.modbuspal2.binding.Binding;
import org.scadalts.modbuspal2.binding.Binding_FLOAT32;
import org.scadalts.modbuspal2.binding.Binding_SINT16;
import org.scadalts.modbuspal2.binding.Binding_SINT32;
import org.scadalts.modbuspal2.generator.Generator;
import org.scadalts.modbuspal2.generator.linear.LinearGenerator;
import org.scadalts.modbuspal2.generator.linear.NoGuiLinearGenerator;
import org.scadalts.modbuspal2.generator.random.RandomGenerator;
import org.scadalts.modbuspal2.generator.sine.NoGuiSineGenerator;
import org.scadalts.modbuspal2.generator.sine.SineGenerator;
import org.scadalts.modbuspal2.instanciator.InstantiableManager;
import org.scadalts.modbuspal2.link.ModbusSerialLink;
import org.scadalts.modbuspal2.master.ModbusMasterTask;
import org.scadalts.modbuspal2.script.ScriptListener;
import org.scadalts.modbuspal2.script.ScriptRunner;
...
import org.scadalts.modbuspal2.toolkit.FileTools;
...
public final class ModbusPalProject
    ...
    private static final org.apache.logging.log4j.Logger LOG = LogManager.getLogger(ModbusPalProject2.class);
    ...
    public ModbusPalProject(boolean noGui) {
        bindingFactory.add(new Binding_SINT16());
        bindingFactory.add(new Binding_SINT32());
        bindingFactory.add(new Binding_FLOAT32());
        if (noGui) {
            generatorFactory.add(new NoGuiLinearGenerator());
            generatorFactory.add(new NoGuiLinearGenerator());
            generatorFactory.add(new NoGuiSineGenerator());
        } else {
            generatorFactory.add(new LinearGenerator());
            generatorFactory.add(new RandomGenerator());
            generatorFactory.add(new SineGenerator());
        }

    }

    private ModbusPalProject(Document doc, File source, boolean noGui)
            throws InstantiationException, IllegalAccessException {
        this(noGui);

        if (noGui) {
            addModbusPalListener(new ModbusPalListenerImpl());
        }
        // get the root node
     ...
     */
    public static ModbusPalProject load(File source, boolean noGui)
        ...
        ModbusPalProject project = new ModbusPalProject(doc, source, noGui);
        return project;
    }
    ...
    private void loadAutomations(Document doc, boolean noGui)
            throws InstantiationException, IllegalAccessException {
        NodeList automationsList = doc.getElementsByTagName("automation");
        LOG.debug("Automations loading... {}", automationsList.getLength());
        for (int i = 0; i < automationsList.getLength(); i++) {
            ...
            automation.loadGenerators(automationNode.getChildNodes(), generatorFactory, noGui);
    ...
    private void loadBinding(Node node, ModbusSlave slave)
        ...
        if (parentAddress == null) {
            ...
            if (parentAddress == null) {
                LOG.info("Cannot bind automation. Parent is neither a register or coil!");
                return;
            }
        }
        ...
        if (slave == null) {
            ...
            if (slaveAddress != null) {
                ...
                try {
                    ...
                    LOG.info("Unable to get Modbus Slave IP address from slave address: " + slaveAddress);
                }
            }
        }
