...
import org.scadalts.modbuspal2.generator.GeneratorListener;
import org.scadalts.modbuspal2.generator.linear.NoGuiLinearGenerator;
import org.scadalts.modbuspal2.generator.random.NoGuiRandomGenerator;
...
import org.scadalts.modbuspal2.toolkit.InstanceCounter;
...
public class Automation implements Runnable {
     ...
     */
    public void loadGenerators(NodeList nodes, InstantiableManager<Generator> gf, boolean noGui) throws InstantiationException, IllegalAccessException {
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (node.getNodeName().compareTo("generator") == 0) {
                ...
                if (gen == null && noGui) {
                    if (className.contains("SineGenerator")) {
                        gen = new NoGuiSineGenerator();
                    } else if (className.contains("LinearGenerator")) {
                        gen = new NoGuiLinearGenerator();
                    } else if (className.contains("RandomGenerator")) {
                        gen = new NoGuiRandomGenerator();
                    }
                }
    ...
    private String createOpenTag() {
        ...
        tag.append(" step=\"").append(stepDelay).append("\"");
        tag.append(" loop=\"").append(loop).append("\"");
        tag.append(" init=\"").append(initialValue).append("\"");
        tag.append(">\r\n");
        return tag.toString();
    }
